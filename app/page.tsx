...
setUsername(value); setUsernameStatus('idle');
if (debounceRef.current) clearTimeout(debounceRef.current);
if (value.length < 3) return;
setUsernameStatus('checking');
debounceRef.current = setTimeout(async () => {
try { const res = await fetch('/api/check-user?username=' + encodeURIComponent(value)); const data = await res.json(); setUsernameStatus(data.found ? 'found' : 'notfound'); } catch { setUsernameStatus('notfound'); }
}, 600);
}, []);
...
{usernameStatus === 'found' && (<button style={{ width: '100%', background: GOLD, color: '#000', border: 'none', padding: '15px', borderRadius: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: 'Montserrat, sans-serif', marginBottom: '15px' }}>Acceder a mon espace</button>)}
<div style={{ display: 'flex', gap: '15px', marginTop: '15px' }}>
<button style={{ flex: 1, padding: '12px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', color: 'white', cursor: 'pointer', fontFamily: 'Montserrat, sans-serif' }}>🌐 Google</button>
<button style={{ flex: 1, padding: '12px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', color: 'white', cursor: 'pointer', fontFamily: 'Montserrat, sans-serif' }}>🍎 Apple</button>
</div>
</div>
) : (
<div>
...
<motion.div key={product.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} style={{ ...glass, padding: '25px', cursor: 'pointer', position: 'relative', transition: 'all 0.3s' }} onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.border = '1px solid rgba(255,140,0,0.5)'; (e.currentTarget as HTMLDivElement).style.transform = 'translateY(-5px)'; }} onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.border = '1px solid rgba(212,175,55,0.2)'; (e.currentTarget as HTMLDivElement).style.transform = 'translateY(0)'; }}>
...
<button onClick={() => { setSelectedProduct(product); }} style={{ flex: 1, padding: '12px', background: 'transparent', border: '1px solid rgba(0,255,65,0.4)', borderRadius: '12px', color: '#00ff41', cursor: 'pointer', fontFamily: 'Montserrat, sans-serif', fontSize: '0.85rem' }}>Test 24H</button>
</div>
</motion.div>
))}
</div>
...
<div style={{ fontSize: '0.75rem', opacity: 0.6, marginBottom: '4px' }}>Expire dans</div>
<div style={{ color: '#00ff41', fontFamily: 'monospace' }}>{testResult.expiresIn}</div>
</div>
</div>
{testResult.qrCode && (<img src={testResult.qrCode} alt="QR" style={{ width: '150px', height: '150px', borderRadius: '12px', marginBottom: '20px', border: '2px solid rgba(0,255,65,0.3)' }} />)}
<div style={{ display: 'flex', gap: '10px' }}>
<button onClick={() => { navigator.clipboard.writeText('Username: ' + testResult.username + '\nPassword: ' + testResult.password + '\nURL: ' + testResult.m3u); toast.success('Copie !'); }} style={{ flex: 1, padding: '12px', background: 'rgba(0,255,65,0.1)', border: '1px solid rgba(0,255,65,0.3)', borderRadius: '12px', color: '#00ff41', cursor: 'pointer', fontFamily: 'Montserrat, sans-serif', fontWeight: 700 }}>Copier</button>
...
<div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, height: '32px', background: '#000', borderTop: '1px solid #00ff41', zIndex: 999999, overflow: 'hidden', display: 'flex', alignItems: 'center' }}>
<div style={{ whiteSpace: 'nowrap', color: '#00ff41', fontSize: '13px', fontWeight: 700, animation: 'scroll 20s linear infinite', paddingLeft: '100%', textShadow: '0 0 5px #00ff41' }}>
